...
export class Context {
  ...
  anchor(variantID: VariantID): VariantID {
    for (const [parent, parentVariant] of this.gates.values()) {
      if (parent.anchored === true) {
        return Math.min(parentVariant.id, variantID);
...
export class Gate {
  ...
  variant(ctx: Context, callback: VariantClosure): void {
    ...
    const anchor = ctx.anchor(variantID);
    if (variantID > anchor) {
      return;
    }
