...
type VariantClosure = (ctx: Context, variant: VariantView) => void;
...
export class Gate {
  ...
  * enumerateVariants(): Iterable<Variant> {
  ...
  tokens(switches: Switches): List<Token> {
    const index = switches.get(this.id, 0);
    const variant = this.variants.get(index, List());
    return variant.flatMap(n => n.tokens(switches));
  }
