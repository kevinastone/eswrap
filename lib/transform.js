...
function walkBinaryOperator(ctx: Context, writer: NodeWriter, lhs: AstVisitor, op: AstVisitor, rhs: AstVisitor): Context {
  const gate = new Gate();
...
function walk(ctx: Context, writer: NodeWriter, node: AstNode): Context {
  ...
  // Expressions
    ...
    ctx = walkBinaryOperator(
      ctx,
      writer,
      (ctx, writer) => walk(ctx, writer, node.left),
      (ctx, writer) => { writer.append(text(node.operator)); return ctx; },
      (ctx, writer) => walk(ctx, writer, node.right),
    );
    ...
    ctx = walk(ctx, writer, node.test);
    writer.append(space());
    writer.append(text('?'));
    writer.append(space());
    ctx = walk(ctx, writer, node.consequent);
    writer.append(space());
    writer.append(text(':'));
    writer.append(space());
    ctx = walk(ctx, writer, node.alternate);
